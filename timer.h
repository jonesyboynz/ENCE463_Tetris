/*
 * timer.h
 *	Shares basic timer functionality and structures
...
typedef struct timeout_s { //structure for a timeout
	portTickType starting_time;
	int timeout_ms;
} Timeout;

typedef struct timer_s { //structure for a timer
	portTickType reference;
	unsigned long elapsed_ticks;
	int running;
} Timer;

typedef struct game_stats_debug_timer_s { //structure for the debug timers and debug timer data that the game uses.
